...
function checkComplexDependency(questionIdToAnswerMap, dependentOn, sanctionExposure) {
  ...
  const checkCondition = (condition) => {
    ...
    if (condition.or) {
      return condition.or.some(checkCondition);
    }
  ...
  return (
    <div className="table-responsive">
      ...
      <div>
        ...
        <ul>
          {Object.entries(filteredReasons).map(([questionId, reasons]) => (
            <li key={questionId}>
              <strong>{questionId}:</strong>
              <ul>
              {reasons.map((reason, index) => <li key={index}>{reason}</li>)} {/* Use reasons instead of filteredReasons[questionData.questionId]*/}
              </ul>
            </li>
          ))}
        </ul>
